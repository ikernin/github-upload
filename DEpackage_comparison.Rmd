...
condition <- relevel(condition, ref = "neg")
sex <- coldata$sex
ageBin <- coldata$ageBin

designMat <- model.matrix(~condition + sex + ageBin)

## estimate commonn + tagwise + trended dispersions
...
results_limma <- topTable(fit, coef = "conditionpos", number = Inf, adjust.method = "BH", sort.by = "logFC")
beep(2)
```
...
venn.diagram(x = list(rownames(subset(results_edgeR$table, (FDR <=0.05 & abs(logFC) > 0.58))), rownames(subset(res_covid2, (padj <=0.05 & abs(log2FoldChange) > 0.58))), rownames(subset(results_limma, (adj.P.Val <=0.05 & abs(logFC) > 0.58)))), 
             category.names = c("edgeR", "DESeq2", "limma"),
             filename = 'venn_DEpackages2.png',
             output = TRUE,
             fill = myCol)

## create a simpler venn diagram to view in R
venn<- list(edger = rownames(subset(results_edgeR$table, (FDR <=0.05 & abs(logFC) > 0.58))), deseq2 = rownames(subset(res_covid2, (padj <=0.05 & abs(log2FoldChange) > 0.58))), limma = rownames(subset(results_limma, (adj.P.Val <=0.05 & abs(logFC) > 0.58))))
